...
function genPage(type: string, queryString: any, qField: string = 'name'): any {
  ...
  if (queryString.q) {
    data = data.filter((datax: any) => datax[qField].indexOf(queryString.q) > -1);
  }
  return {
    total: data.length,
...
function projectGet(id: number): any {
  ...
  const item = {
    ...
    tasks: DATA.task,
    discussions: new Array(Random.natural(3, 8)).fill({}).map((v, idx1) => ({
      id: idx1 + 1,
      user_avatar: genMp(),
      user_name: genName(),
    ...
    activities: new Array(Random.natural(3, 12)).fill({}).map((v, idx2) => ({
      id: idx2 + 1,
      user_avatar: genMp(),
      user_name: genName(),
      time: Random.datetime(),
      type: idx2 % 2 === 0 ? genArr(['add', 'completed', 'assigned']) : 'push',
...
function billingGet(id: number): any {
  ...
  const item = {
    ...DATA.billing[idx],
    messages: new Array(Random.natural(0, 5)).fill({}).map((v, idx1) => ({
      id: idx1 + 1,
...
function faq(): any {
  ...
  DATA.faq = new Array(6).fill({}).map((v, idx) => ({
    ...
    children: new Array(Random.natural(3, 6)).fill({}).map((v1, idx1) => ({
      active: idx1 === 0,
