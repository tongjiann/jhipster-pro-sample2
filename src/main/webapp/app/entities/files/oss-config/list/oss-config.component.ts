...
@Component({
  selector: 'jhi-oss-config',
  templateUrl: './oss-config.component.html',
  styles: [
    `
      .search-box {
        padding: 8px;
...
export class OssConfigComponent implements OnInit {
  ...
  ossConfigCommonTableData: ICommonTable | null = null; // 实体模型数据
  ...
  previousPage: any;
  constructor(
    protected ossConfigService: OssConfigService,
    ...
    /* this.routeData = this.activatedRoute?.data?.subscribe((data) => {
            ...
            this.ossConfigCommonTableData = data.commonTableData[0];
        }); */
  }
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.ossConfigService.query(requestOption).subscribe(
      (res: HttpResponse<IOssConfig[]>) => {
  ...
  ngOnInit(): void {
    ...
    this.translateService.get('jhipsterSampleApplicationApp.OssProvider.MINIO').subscribe(text => {
      listOfFilterprovider.push({ text, value: 'MINIO' });
    });
    this.translateService.get('jhipsterSampleApplicationApp.OssProvider.QINIU').subscribe(text => {
      listOfFilterprovider.push({ text, value: 'QINIU' });
    });
    this.translateService.get('jhipsterSampleApplicationApp.OssProvider.ALI').subscribe(text => {
      listOfFilterprovider.push({ text, value: 'ALI' });
    });
    this.translateService.get('jhipsterSampleApplicationApp.OssProvider.TENCENT').subscribe(text => {
      listOfFilterprovider.push({ text, value: 'TENCENT' });
    });
    this.translateService.get('jhipsterSampleApplicationApp.OssProvider.LOCAL').subscribe(text => {
      listOfFilterprovider.push({ text, value: 'LOCAL' });
    });
    this.mapOfFilter.provider = { list: listOfFilterprovider, value: [], type: 'Enum' };
  }
  trackId(index: number, item: IOssConfig) {
    return item.id;
  }
  ...
  delete(id: number | undefined): void {
    ...
    this.ossConfigService.delete(id).subscribe(
  ...
  deleteByIds(ids: number[]): void {
    this.ossConfigService.deleteByIds(ids).subscribe(
      (res: HttpResponse<any>) => {
  ...
  protected paginateOssConfigs(data: IOssConfig[], headers: HttpHeaders) {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.queryCount = this.totalItems;
    // 设置每行为不可编辑状态，设置单选框未选状态
    data.forEach(ossConfig => {
      this.editStatus[ossConfig.id!] = {
        edit: false,
      };
      this.mapOfCheckedId[ossConfig.id!] = false;
    });
    this.ossConfigs = data;
  }
  ...
  saveEdit(ossConfig: IOssConfig) {
    this.isSaving = true;
    if (ossConfig.id !== undefined) {
      if (this.omitFields.length > 1) {
        this.subscribeToSaveResponse(this.ossConfigService.updateBySpecifiedFields(ossConfig, this.omitFields));
      } else {
        this.subscribeToSaveResponse(this.ossConfigService.update(ossConfig));
      }
    } else {
      this.subscribeToSaveResponse(this.ossConfigService.create(ossConfig));
    }
    this.editStatus[ossConfig.id!].edit = false;
  }
  ...
  // 新增 并设置为可编辑状态
  newOssConfig() {
    const ossConfig = new OssConfig();
    ossConfig.id = -1;
    this.ossConfigs?.push(ossConfig);
    this.editStatus[ossConfig.id].edit = true;
  }
  // 设置某行可编辑
  startEdit(ossConfig: IOssConfig): void {
    this.editStatus[ossConfig.id!].edit = true;
  }

  protected subscribeToSaveResponse(result: Observable<HttpResponse<IOssConfig>>) {
    result.subscribe(
      (res: HttpResponse<IOssConfig>) => this.onSaveSuccess(),
  ...
  // 处理单选框状态
  refreshStatus(): void {
    this.isAllDisplayDataChecked = this.ossConfigs?.every(item => this.mapOfCheckedId[item.id!])!;
    this.isIndeterminate = this.ossConfigs?.some(item => this.mapOfCheckedId[item.id!])! && !this.isAllDisplayDataChecked;
    this.numberOfChecked = this.ossConfigs?.filter(item => this.mapOfCheckedId[item.id!]).length!;
  }

  checkAll(value: boolean): void {
    this.ossConfigs?.forEach(item => (this.mapOfCheckedId[item.id!] = value));
    this.refreshStatus();
  }
  ...
  protected onSuccess(data: IOssConfig[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/oss-config'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
  ...
  showField(fieldName: string): boolean {
    const fields = this.ossConfigCommonTableData?.commonTableFields;
    if (!fields) {
      return true;
    }
  ...
  showRelationship(relationshipName: string): boolean {
    const relationships = this.ossConfigCommonTableData?.relationships;
    if (!relationships) {
      return true;
    }
  ...
  fieldWidth(fieldName: string): string {
    if (!this.ossConfigCommonTableData) {
      return '100px';
    }
    return calculateFieldWidth(this.ossConfigCommonTableData, fieldName);
  }
  ...
  relationshipWidth(relationshipName: string): string {
    ...
    return calculateRelationshipWidth(this.ossConfigCommonTableData, relationshipName);
  }

  tableWidth(): string {
    return calculateTableWidth(this.ossConfigCommonTableData!);
  }
}
