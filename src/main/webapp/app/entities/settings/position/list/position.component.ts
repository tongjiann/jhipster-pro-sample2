...
export class PositionComponent implements OnInit {
  ...
  positionCommonTableData: ICommonTable | null = null; // 实体模型数据
  ...
  constructor(
    ...
    /* this.routeData = this.activatedRoute?.data?.subscribe((data) => {
            ...
            this.positionCommonTableData = data.commonTableData[0];
        }); */
  }
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.positionService.query(requestOption).subscribe(
  ...
  deleteByIds(ids: number[]): void {
    this.positionService.deleteByIds(ids).subscribe(
      (res: HttpResponse<any>) => {
  ...
  protected paginatePositions(data: IPosition[], headers: HttpHeaders) {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.queryCount = this.totalItems;
    // 设置每行为不可编辑状态，设置单选框未选状态
    data.forEach(position => {
      this.editStatus[position.id!] = {
        edit: false,
      };
      this.mapOfCheckedId[position.id!] = false;
    });
    this.positions = data;
  }
  ...
  saveEdit(position: IPosition) {
    this.isSaving = true;
    if (position.id !== undefined) {
      if (this.omitFields.length > 1) {
        this.subscribeToSaveResponse(this.positionService.updateBySpecifiedFields(position, this.omitFields));
      } else {
    ...
    this.editStatus[position.id!].edit = false;
  }
  ...
  newPosition() {
    ...
    this.editStatus[position.id].edit = true;
  }
  // 设置某行可编辑
  startEdit(position: IPosition): void {
    this.editStatus[position.id!].edit = true;
  }

  protected subscribeToSaveResponse(result: Observable<HttpResponse<IPosition>>) {
    result.subscribe(
      (res: HttpResponse<IPosition>) => this.onSaveSuccess(),
  ...
  // 处理单选框状态
  refreshStatus(): void {
    this.isAllDisplayDataChecked = this.positions?.every(item => this.mapOfCheckedId[item.id!])!;
    this.isIndeterminate = this.positions?.some(item => this.mapOfCheckedId[item.id!])! && !this.isAllDisplayDataChecked;
    this.numberOfChecked = this.positions?.filter(item => this.mapOfCheckedId[item.id!]).length!;
  }

  checkAll(value: boolean): void {
    this.positions?.forEach(item => (this.mapOfCheckedId[item.id!] = value));
    this.refreshStatus();
  }
  ...
  showField(fieldName: string): boolean {
    const fields = this.positionCommonTableData?.commonTableFields;
    if (!fields) {
      return true;
    }
  ...
  showRelationship(relationshipName: string): boolean {
    const relationships = this.positionCommonTableData?.relationships;
    if (!relationships) {
      return true;
    }
  ...
  fieldWidth(fieldName: string): string {
    if (!this.positionCommonTableData) {
      return '100px';
    }
    return calculateFieldWidth(this.positionCommonTableData, fieldName);
  }
  ...
  relationshipWidth(relationshipName: string): string {
    ...
    return calculateRelationshipWidth(this.positionCommonTableData, relationshipName);
  }

  tableWidth(): string {
    return calculateTableWidth(this.positionCommonTableData!);
  }
}
