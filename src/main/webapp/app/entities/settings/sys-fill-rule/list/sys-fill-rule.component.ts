...
@Component({
  selector: 'jhi-sys-fill-rule',
  templateUrl: './sys-fill-rule.component.html',
  styles: [
    `
      .search-box {
        padding: 8px;
...
export class SysFillRuleComponent implements OnInit {
  ...
  sysFillRuleCommonTableData: ICommonTable | null = null; // 实体模型数据
  ...
  previousPage: any;
  constructor(
    protected sysFillRuleService: SysFillRuleService,
    ...
    /* this.routeData = this.activatedRoute?.data?.subscribe((data) => {
            ...
            this.sysFillRuleCommonTableData = data.commonTableData[0];
        }); */
  }
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.sysFillRuleService.query(requestOption).subscribe(
      (res: HttpResponse<ISysFillRule[]>) => {
  ...
  trackId(index: number, item: ISysFillRule) {
    return item.id;
  }
  ...
  delete(id: number | undefined): void {
    ...
    this.sysFillRuleService.delete(id).subscribe(
  ...
  deleteByIds(ids: number[]): void {
    this.sysFillRuleService.deleteByIds(ids).subscribe(
      (res: HttpResponse<any>) => {
  ...
  protected paginateSysFillRules(data: ISysFillRule[], headers: HttpHeaders) {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.queryCount = this.totalItems;
    // 设置每行为不可编辑状态，设置单选框未选状态
    data.forEach(sysFillRule => {
      this.editStatus[sysFillRule.id!] = {
        edit: false,
      };
      this.mapOfCheckedId[sysFillRule.id!] = false;
    });
    this.sysFillRules = data;
  }
  ...
  saveEdit(sysFillRule: ISysFillRule) {
    this.isSaving = true;
    if (sysFillRule.id !== undefined) {
      if (this.omitFields.length > 1) {
        this.subscribeToSaveResponse(this.sysFillRuleService.updateBySpecifiedFields(sysFillRule, this.omitFields));
      } else {
        this.subscribeToSaveResponse(this.sysFillRuleService.update(sysFillRule));
      }
    } else {
      this.subscribeToSaveResponse(this.sysFillRuleService.create(sysFillRule));
    }
    this.editStatus[sysFillRule.id!].edit = false;
  }
  ...
  // 新增 并设置为可编辑状态
  newSysFillRule() {
    const sysFillRule = new SysFillRule();
    sysFillRule.id = -1;
    this.sysFillRules?.push(sysFillRule);
    this.editStatus[sysFillRule.id].edit = true;
  }
  // 设置某行可编辑
  startEdit(sysFillRule: ISysFillRule): void {
    this.editStatus[sysFillRule.id!].edit = true;
  }

  protected subscribeToSaveResponse(result: Observable<HttpResponse<ISysFillRule>>) {
    result.subscribe(
      (res: HttpResponse<ISysFillRule>) => this.onSaveSuccess(),
  ...
  // 处理单选框状态
  refreshStatus(): void {
    this.isAllDisplayDataChecked = this.sysFillRules?.every(item => this.mapOfCheckedId[item.id!])!;
    this.isIndeterminate = this.sysFillRules?.some(item => this.mapOfCheckedId[item.id!])! && !this.isAllDisplayDataChecked;
    this.numberOfChecked = this.sysFillRules?.filter(item => this.mapOfCheckedId[item.id!]).length!;
  }

  checkAll(value: boolean): void {
    this.sysFillRules?.forEach(item => (this.mapOfCheckedId[item.id!] = value));
    this.refreshStatus();
  }
  ...
  protected onSuccess(data: ISysFillRule[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/sys-fill-rule'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
  ...
  showField(fieldName: string): boolean {
    const fields = this.sysFillRuleCommonTableData?.commonTableFields;
    if (!fields) {
      return true;
    }
  ...
  showRelationship(relationshipName: string): boolean {
    const relationships = this.sysFillRuleCommonTableData?.relationships;
    if (!relationships) {
      return true;
    }
  ...
  fieldWidth(fieldName: string): string {
    if (!this.sysFillRuleCommonTableData) {
      return '100px';
    }
    return calculateFieldWidth(this.sysFillRuleCommonTableData, fieldName);
  }
  ...
  relationshipWidth(relationshipName: string): string {
    ...
    return calculateRelationshipWidth(this.sysFillRuleCommonTableData, relationshipName);
  }

  tableWidth(): string {
    return calculateTableWidth(this.sysFillRuleCommonTableData!);
  }
}
