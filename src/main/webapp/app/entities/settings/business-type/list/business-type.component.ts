...
export class BusinessTypeComponent implements OnInit {
  ...
  businessTypeCommonTableData: ICommonTable | null = null; // 实体模型数据
  ...
  constructor(
    ...
    /* this.routeData = this.activatedRoute?.data?.subscribe((data) => {
            ...
            this.businessTypeCommonTableData = data.commonTableData[0];
        }); */
  }
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.businessTypeService.query(requestOption).subscribe(
      (res: HttpResponse<IBusinessType[]>) => {
  ...
  deleteByIds(ids: number[]): void {
    this.businessTypeService.deleteByIds(ids).subscribe(
      (res: HttpResponse<any>) => {
  ...
  protected paginateBusinessTypes(data: IBusinessType[], headers: HttpHeaders) {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.queryCount = this.totalItems;
    // 设置每行为不可编辑状态，设置单选框未选状态
    data.forEach(businessType => {
      this.editStatus[businessType.id!] = {
        edit: false,
      };
      this.mapOfCheckedId[businessType.id!] = false;
    });
  ...
  saveEdit(businessType: IBusinessType) {
    this.isSaving = true;
    if (businessType.id !== undefined) {
      if (this.omitFields.length > 1) {
        this.subscribeToSaveResponse(this.businessTypeService.updateBySpecifiedFields(businessType, this.omitFields));
      } else {
    ...
    this.editStatus[businessType.id!].edit = false;
  }
  ...
  newBusinessType() {
    ...
    this.editStatus[businessType.id].edit = true;
  }
  // 设置某行可编辑
  startEdit(businessType: IBusinessType): void {
    this.editStatus[businessType.id!].edit = true;
  }

  protected subscribeToSaveResponse(result: Observable<HttpResponse<IBusinessType>>) {
    result.subscribe(
      (res: HttpResponse<IBusinessType>) => this.onSaveSuccess(),
  ...
  // 处理单选框状态
  refreshStatus(): void {
    this.isAllDisplayDataChecked = this.businessTypes?.every(item => this.mapOfCheckedId[item.id!])!;
    this.isIndeterminate = this.businessTypes?.some(item => this.mapOfCheckedId[item.id!])! && !this.isAllDisplayDataChecked;
    this.numberOfChecked = this.businessTypes?.filter(item => this.mapOfCheckedId[item.id!]).length!;
  }

  checkAll(value: boolean): void {
    this.businessTypes?.forEach(item => (this.mapOfCheckedId[item.id!] = value));
    this.refreshStatus();
  }
  ...
  protected onSuccess(data: IBusinessType[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
  ...
  showField(fieldName: string): boolean {
    const fields = this.businessTypeCommonTableData?.commonTableFields;
    if (!fields) {
      return true;
    }
  ...
  showRelationship(relationshipName: string): boolean {
    const relationships = this.businessTypeCommonTableData?.relationships;
    if (!relationships) {
      return true;
    }
  ...
  fieldWidth(fieldName: string): string {
    if (!this.businessTypeCommonTableData) {
      return '100px';
    }
    return calculateFieldWidth(this.businessTypeCommonTableData, fieldName);
  }
  ...
  relationshipWidth(relationshipName: string): string {
    ...
    return calculateRelationshipWidth(this.businessTypeCommonTableData, relationshipName);
  }

  tableWidth(): string {
    return calculateTableWidth(this.businessTypeCommonTableData!);
  }
}
