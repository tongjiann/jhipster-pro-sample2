...
export class DataDictionaryComponent implements OnInit {
  ...
  dataDictionaryCommonTableData: ICommonTable | null = null; // 实体模型数据
  ...
  constructor(
    ...
    /* this.routeData = this.activatedRoute?.data?.subscribe((data) => {
            ...
            this.dataDictionaryCommonTableData = data.commonTableData[0];
        }); */
  }
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.dataDictionaryService.tree(requestOption).subscribe(
      (res: HttpResponse<IDataDictionary[]>) => {
  ...
  ngOnInit(): void {
    ...
    // 处理enum
    const listOfFiltervalueType: any[] = [];
    this.translateService.get('jhipsterSampleApplicationApp.DictType.NUMBER').subscribe(text => {
      listOfFiltervalueType.push({ text, value: 'NUMBER' });
    });
    this.translateService.get('jhipsterSampleApplicationApp.DictType.STRING').subscribe(text => {
      listOfFiltervalueType.push({ text, value: 'STRING' });
    });
    this.mapOfFilter.valueType = { list: listOfFiltervalueType, value: [], type: 'Enum' };
  }

  trackDataDictionaryById(index: number, item: IDataDictionary) {
    return item.id;
  }
  trackId(index: number, item: IDataDictionary) {
    return item.id;
  }
  ...
  delete(id: number | undefined): void {
    ...
    this.dataDictionaryService.delete(id).subscribe(
  ...
  deleteByIds(ids: number[]): void {
    this.dataDictionaryService.deleteByIds(ids).subscribe(
      (res: HttpResponse<any>) => {
  ...
  protected paginateDataDictionaries(data: IDataDictionary[], headers: HttpHeaders) {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.queryCount = this.totalItems;
    // 设置每行为不可编辑状态，设置单选框未选状态
    data.forEach(dataDictionary => {
      this.editStatus[dataDictionary.id!] = {
        edit: false,
      };
      this.mapOfCheckedId[dataDictionary.id!] = false;
    });
    this.dataDictionaries = data;

    // treetable 5.
    this.dataDictionaries.forEach(item => {
  ...
  // treetable 2.
  collapse(array: IDataDictionary[], data: IDataDictionary, $event: boolean): void {
    if ($event === false) {
      if (data.children) {
  ...
  // treetable 3.
  convertTreeToList(root: any): IDataDictionary[] {
    const stack = [];
    const tempArray: any[] = [];
    const hashMap = {};
  ...
  // treetable  4.
  visitNode(node: IDataDictionary, hashMap: any, array: IDataDictionary[]): void {
    if (!hashMap[node.id!]) {
      hashMap[node.id!] = true;
      array.push(node);
    }
  }
  saveEdit(dataDictionary: IDataDictionary) {
    this.isSaving = true;
    if (dataDictionary.id !== undefined) {
      if (this.omitFields.length > 1) {
        this.subscribeToSaveResponse(this.dataDictionaryService.updateBySpecifiedFields(dataDictionary, this.omitFields));
      } else {
        this.subscribeToSaveResponse(this.dataDictionaryService.update(dataDictionary));
      }
    } else {
      this.subscribeToSaveResponse(this.dataDictionaryService.create(dataDictionary));
    }
    this.editStatus[dataDictionary.id!].edit = false;
  }
  ...
  newDataDictionary() {
    ...
    dataDictionary.id = -1;
    this.dataDictionaries?.push(dataDictionary);
    this.editStatus[dataDictionary.id].edit = true;
  }
  // 设置某行可编辑
  startEdit(dataDictionary: IDataDictionary): void {
    this.editStatus[dataDictionary.id!].edit = true;
  }

  protected subscribeToSaveResponse(result: Observable<HttpResponse<IDataDictionary>>) {
    result.subscribe(
      (res: HttpResponse<IDataDictionary>) => this.onSaveSuccess(),
  ...
  // 处理单选框状态
  refreshStatus(): void {
    this.isAllDisplayDataChecked = this.dataDictionaries?.every(item => this.mapOfCheckedId[item.id!])!;
    this.isIndeterminate = this.dataDictionaries?.some(item => this.mapOfCheckedId[item.id!])! && !this.isAllDisplayDataChecked;
    this.numberOfChecked = this.dataDictionaries?.filter(item => this.mapOfCheckedId[item.id!]).length!;
  }

  checkAll(value: boolean): void {
    this.dataDictionaries?.forEach(item => (this.mapOfCheckedId[item.id!] = value));
    this.refreshStatus();
  }
  ...
  protected onSuccess(data: IDataDictionary[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/data-dictionary'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
  ...
  showField(fieldName: string): boolean {
    const fields = this.dataDictionaryCommonTableData?.commonTableFields;
    if (!fields) {
      return true;
    }
  ...
  showRelationship(relationshipName: string): boolean {
    const relationships = this.dataDictionaryCommonTableData?.relationships;
    if (!relationships) {
      return true;
    }
  ...
  fieldWidth(fieldName: string): string {
    if (!this.dataDictionaryCommonTableData) {
      return '100px';
    }
    return calculateFieldWidth(this.dataDictionaryCommonTableData, fieldName);
  }
  ...
  relationshipWidth(relationshipName: string): string {
    ...
    return calculateRelationshipWidth(this.dataDictionaryCommonTableData, relationshipName);
  }

  tableWidth(): string {
    return calculateTableWidth(this.dataDictionaryCommonTableData!);
  }
}
