...
export class SmsMessageComponent implements OnInit {
  ...
  smsMessageCommonTableData: ICommonTable | null = null; // 实体模型数据
  ...
  constructor(
    ...
    /* this.routeData = this.activatedRoute?.data?.subscribe((data) => {
            ...
            this.smsMessageCommonTableData = data.commonTableData[0];
        }); */
  }
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.smsMessageService.query(requestOption).subscribe(
      (res: HttpResponse<ISmsMessage[]>) => {
  ...
  ngOnInit(): void {
    ...
    // 处理enum
    const listOfFiltersendType: any[] = [];
    this.translateService.get('jhipsterSampleApplicationApp.MessageSendType.EMAIL').subscribe(text => {
      listOfFiltersendType.push({ text, value: 'EMAIL' });
    });
    this.translateService.get('jhipsterSampleApplicationApp.MessageSendType.SMS').subscribe(text => {
      listOfFiltersendType.push({ text, value: 'SMS' });
    });
    this.translateService.get('jhipsterSampleApplicationApp.MessageSendType.WECHAT').subscribe(text => {
      listOfFiltersendType.push({ text, value: 'WECHAT' });
    });
    this.mapOfFilter.sendType = { list: listOfFiltersendType, value: [], type: 'Enum' };

    const listOfFiltersendStatus: any[] = [];
    this.translateService.get('jhipsterSampleApplicationApp.SendStatus.WAITING').subscribe(text => {
      listOfFiltersendStatus.push({ text, value: 'WAITING' });
    });
    this.translateService.get('jhipsterSampleApplicationApp.SendStatus.SUCCESS').subscribe(text => {
      listOfFiltersendStatus.push({ text, value: 'SUCCESS' });
    });
    this.translateService.get('jhipsterSampleApplicationApp.SendStatus.FAILURE').subscribe(text => {
      listOfFiltersendStatus.push({ text, value: 'FAILURE' });
    });
    this.translateService.get('jhipsterSampleApplicationApp.SendStatus.NOT_TRY').subscribe(text => {
      listOfFiltersendStatus.push({ text, value: 'NOT_TRY' });
    });
    this.mapOfFilter.sendStatus = { list: listOfFiltersendStatus, value: [], type: 'Enum' };
  }
  trackId(index: number, item: ISmsMessage) {
    return item.id;
  }
  ...
  delete(id: number | undefined): void {
    ...
    this.smsMessageService.delete(id).subscribe(
  ...
  deleteByIds(ids: number[]): void {
    this.smsMessageService.deleteByIds(ids).subscribe(
      (res: HttpResponse<any>) => {
  ...
  protected paginateSmsMessages(data: ISmsMessage[], headers: HttpHeaders) {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.queryCount = this.totalItems;
    // 设置每行为不可编辑状态，设置单选框未选状态
    data.forEach(smsMessage => {
      this.editStatus[smsMessage.id!] = {
        edit: false,
      };
      this.mapOfCheckedId[smsMessage.id!] = false;
    });
  ...
  saveEdit(smsMessage: ISmsMessage) {
    this.isSaving = true;
    if (smsMessage.id !== undefined) {
      if (this.omitFields.length > 1) {
        this.subscribeToSaveResponse(this.smsMessageService.updateBySpecifiedFields(smsMessage, this.omitFields));
      } else {
        this.subscribeToSaveResponse(this.smsMessageService.update(smsMessage));
      }
    } else {
      this.subscribeToSaveResponse(this.smsMessageService.create(smsMessage));
    }
    this.editStatus[smsMessage.id!].edit = false;
  }
  ...
  newSmsMessage() {
    ...
    this.editStatus[smsMessage.id].edit = true;
  }
  // 设置某行可编辑
  startEdit(smsMessage: ISmsMessage): void {
    this.editStatus[smsMessage.id!].edit = true;
  }

  protected subscribeToSaveResponse(result: Observable<HttpResponse<ISmsMessage>>) {
    result.subscribe(
      (res: HttpResponse<ISmsMessage>) => this.onSaveSuccess(),
  ...
  // 处理单选框状态
  refreshStatus(): void {
    this.isAllDisplayDataChecked = this.smsMessages?.every(item => this.mapOfCheckedId[item.id!])!;
    this.isIndeterminate = this.smsMessages?.some(item => this.mapOfCheckedId[item.id!])! && !this.isAllDisplayDataChecked;
    this.numberOfChecked = this.smsMessages?.filter(item => this.mapOfCheckedId[item.id!]).length!;
  }

  checkAll(value: boolean): void {
    this.smsMessages?.forEach(item => (this.mapOfCheckedId[item.id!] = value));
    this.refreshStatus();
  }
  ...
  protected onSuccess(data: ISmsMessage[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/sms-message'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
  ...
  showField(fieldName: string): boolean {
    const fields = this.smsMessageCommonTableData?.commonTableFields;
    if (!fields) {
      return true;
    }
  ...
  showRelationship(relationshipName: string): boolean {
    const relationships = this.smsMessageCommonTableData?.relationships;
    if (!relationships) {
      return true;
    }
  ...
  fieldWidth(fieldName: string): string {
    if (!this.smsMessageCommonTableData) {
      return '100px';
    }
    return calculateFieldWidth(this.smsMessageCommonTableData, fieldName);
  }
  ...
  relationshipWidth(relationshipName: string): string {
    ...
    return calculateRelationshipWidth(this.smsMessageCommonTableData, relationshipName);
  }

  tableWidth(): string {
    return calculateTableWidth(this.smsMessageCommonTableData!);
  }
}
