...
@Component({
  selector: 'jhi-u-report-file',
  templateUrl: './u-report-file.component.html',
  styles: [
    `
      .search-box {
        padding: 8px;
...
export class UReportFileComponent implements OnInit {
  ...
  uReportFileCommonTableData: ICommonTable | null = null; // 实体模型数据
  ...
  constructor(
    ...
    /* this.routeData = this.activatedRoute?.data?.subscribe((data) => {
            ...
            this.uReportFileCommonTableData = data.commonTableData[0];
        }); */
  }
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.uReportFileService.query(requestOption).subscribe(
      (res: HttpResponse<IUReportFile[]>) => {
  ...
  trackId(index: number, item: IUReportFile) {
    return item.id;
  }
  ...
  delete(id: number | undefined): void {
    ...
    this.uReportFileService.delete(id).subscribe(
  ...
  deleteByIds(ids: number[]): void {
    this.uReportFileService.deleteByIds(ids).subscribe(
      (res: HttpResponse<any>) => {
  ...
  protected paginateUReportFiles(data: IUReportFile[], headers: HttpHeaders) {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.queryCount = this.totalItems;
    // 设置每行为不可编辑状态，设置单选框未选状态
    data.forEach(uReportFile => {
      this.editStatus[uReportFile.id!] = {
        edit: false,
      };
      this.mapOfCheckedId[uReportFile.id!] = false;
    });
    this.uReportFiles = data;
  }
  ...
  saveEdit(uReportFile: IUReportFile) {
    this.isSaving = true;
    if (uReportFile.id !== undefined) {
      if (this.omitFields.length > 1) {
        this.subscribeToSaveResponse(this.uReportFileService.updateBySpecifiedFields(uReportFile, this.omitFields));
      } else {
        this.subscribeToSaveResponse(this.uReportFileService.update(uReportFile));
      }
    } else {
      this.subscribeToSaveResponse(this.uReportFileService.create(uReportFile));
    }
    this.editStatus[uReportFile.id!].edit = false;
  }
  ...
  // 新增 并设置为可编辑状态
  newUReportFile() {
    const uReportFile = new UReportFile();
    uReportFile.id = -1;
    this.uReportFiles?.push(uReportFile);
    this.editStatus[uReportFile.id].edit = true;
  }
  // 设置某行可编辑
  startEdit(uReportFile: IUReportFile): void {
    this.editStatus[uReportFile.id!].edit = true;
  }

  protected subscribeToSaveResponse(result: Observable<HttpResponse<IUReportFile>>) {
    result.subscribe(
      (res: HttpResponse<IUReportFile>) => this.onSaveSuccess(),
  ...
  // 处理单选框状态
  refreshStatus(): void {
    this.isAllDisplayDataChecked = this.uReportFiles?.every(item => this.mapOfCheckedId[item.id!])!;
    this.isIndeterminate = this.uReportFiles?.some(item => this.mapOfCheckedId[item.id!])! && !this.isAllDisplayDataChecked;
    this.numberOfChecked = this.uReportFiles?.filter(item => this.mapOfCheckedId[item.id!]).length!;
  }

  checkAll(value: boolean): void {
    this.uReportFiles?.forEach(item => (this.mapOfCheckedId[item.id!] = value));
    this.refreshStatus();
  }
  ...
  protected onSuccess(data: IUReportFile[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/u-report-file'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
  ...
  showField(fieldName: string): boolean {
    const fields = this.uReportFileCommonTableData?.commonTableFields;
    if (!fields) {
      return true;
    }
  ...
  showRelationship(relationshipName: string): boolean {
    const relationships = this.uReportFileCommonTableData?.relationships;
    if (!relationships) {
      return true;
    }
  ...
  fieldWidth(fieldName: string): string {
    if (!this.uReportFileCommonTableData) {
      return '100px';
    }
    return calculateFieldWidth(this.uReportFileCommonTableData, fieldName);
  }
  ...
  relationshipWidth(relationshipName: string): string {
    ...
    return calculateRelationshipWidth(this.uReportFileCommonTableData, relationshipName);
  }

  tableWidth(): string {
    return calculateTableWidth(this.uReportFileCommonTableData!);
  }
}
