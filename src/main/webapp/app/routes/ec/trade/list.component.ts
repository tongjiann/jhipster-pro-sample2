...
import { DrawerHelper, _HttpClient } from '@delon/theme';
...
export class ECTradeComponent implements OnInit {
  ...
  btn: SFButton = {
    render: { grid: { span: 24 }, class: 'text-center mb0 mt-md', spanLabelFixed: 0 },
    submit: 'Search',
  };
  ...
  view(i: any): void {
    this.drawer.create(`查看订单 #${i.id}`, ECTradeViewComponent, { i }, { size: 666 }).subscribe(() => this.load());
  }

  memo(i: any): void {
    this.drawer.create(`商家备注`, ECTradeMemoComponent, { i }, { size: 350 }).subscribe((res: any) => {
      i.memo = res.memo;
      this.cd.detectChanges();
    });
  }
