...
export class WareEditService implements OnDestroy {
  ...
  skuLoad(cat: any): Promise<void> {
    return new Promise(resolve => {
      const ls = deepCopy(cat);
      ls.forEach((i: NzSafeAny) => {
        i.value_org = `${i.value}`;
      ...
      for (const i of this.i.skus) {
        const arrtIdArr = i.attributes.split(':').map((v: NzSafeAny) => +v);
  ...
  skuCho(): void {
    ...
    this.colors = colorData?.arr.filter(w => w.sel) ?? [];
  ...
  getSaveData(): {} {
    ...
    // mp
    const wareMainImgs = postData.imgs[0].ls.filter((w: NzSafeAny) => !!w);
    ...
    // imgs
    const imgs: { [key: string]: NzSafeAny } = {};
