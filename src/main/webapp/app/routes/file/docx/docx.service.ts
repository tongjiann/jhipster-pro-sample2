...
export class FileDocxService {
  ...
  private getByHttp(url: string): Promise<any> {
    return this.http.get(url, null, { responseType: 'arraybuffer' }).toPromise();
  }

  async docxTemplate(file: string | File): Promise<void>;
  ...
  async docxTemplate(file: string | File, data: {} | null = null, outputFileName: string = 'output.docx'): Promise<void> {
    await this.init();
  ...
  async htmlToDocx(html: string, options: {} = {}, outputFileName: string = 'output.docx'): Promise<void> {
    await this.init();
