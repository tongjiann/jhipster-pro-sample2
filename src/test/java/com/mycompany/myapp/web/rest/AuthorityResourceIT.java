...
class AuthorityResourceIT {
    ...
    void createAuthority() throws Exception {
        ...
        assertThat(testAuthority.getRemovedAt()).isEqualTo(DEFAULT_REMOVED_AT);
    }
    ...
    @Test
    @Transactional
    void getAllAuthoritiesByRemovedAtIsEqualToSomething() throws Exception {
        // Initialize the database
        authorityRepository.insert(authority);

        // Get all the authorityList where removedAt equals to DEFAULT_REMOVED_AT
        defaultAuthorityShouldBeFound("removedAt.equals=" + DEFAULT_REMOVED_AT);

        // Get all the authorityList where removedAt equals to UPDATED_REMOVED_AT
        defaultAuthorityShouldNotBeFound("removedAt.equals=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllAuthoritiesByRemovedAtIsNotEqualToSomething() throws Exception {
        // Initialize the database
        authorityRepository.insert(authority);

        // Get all the authorityList where removedAt not equals to DEFAULT_REMOVED_AT
        defaultAuthorityShouldNotBeFound("removedAt.notEquals=" + DEFAULT_REMOVED_AT);

        // Get all the authorityList where removedAt not equals to UPDATED_REMOVED_AT
        defaultAuthorityShouldBeFound("removedAt.notEquals=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllAuthoritiesByRemovedAtIsInShouldWork() throws Exception {
        // Initialize the database
        authorityRepository.insert(authority);

        // Get all the authorityList where removedAt in DEFAULT_REMOVED_AT or UPDATED_REMOVED_AT
        defaultAuthorityShouldBeFound("removedAt.in=" + DEFAULT_REMOVED_AT + "," + UPDATED_REMOVED_AT);

        // Get all the authorityList where removedAt equals to UPDATED_REMOVED_AT
        defaultAuthorityShouldNotBeFound("removedAt.in=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllAuthoritiesByRemovedAtIsNullOrNotNull() throws Exception {
        // Initialize the database
        authorityRepository.insert(authority);

        // Get all the authorityList where removedAt is not null
        defaultAuthorityShouldBeFound("removedAt.specified=true");

        // Get all the authorityList where removedAt is null
        defaultAuthorityShouldNotBeFound("removedAt.specified=false");
    }

    @Test
    @Transactional
    void getAllAuthoritiesByRemovedAtIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        authorityRepository.insert(authority);

        // Get all the authorityList where removedAt is greater than or equal to DEFAULT_REMOVED_AT
        defaultAuthorityShouldBeFound("removedAt.greaterThanOrEqual=" + DEFAULT_REMOVED_AT);

        // Get all the authorityList where removedAt is greater than or equal to UPDATED_REMOVED_AT
        defaultAuthorityShouldNotBeFound("removedAt.greaterThanOrEqual=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllAuthoritiesByRemovedAtIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        authorityRepository.insert(authority);

        // Get all the authorityList where removedAt is less than or equal to DEFAULT_REMOVED_AT
        defaultAuthorityShouldBeFound("removedAt.lessThanOrEqual=" + DEFAULT_REMOVED_AT);

        // Get all the authorityList where removedAt is less than or equal to SMALLER_REMOVED_AT
        defaultAuthorityShouldNotBeFound("removedAt.lessThanOrEqual=" + SMALLER_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllAuthoritiesByRemovedAtIsLessThanSomething() throws Exception {
        // Initialize the database
        authorityRepository.insert(authority);

        // Get all the authorityList where removedAt is less than DEFAULT_REMOVED_AT
        defaultAuthorityShouldNotBeFound("removedAt.lessThan=" + DEFAULT_REMOVED_AT);

        // Get all the authorityList where removedAt is less than UPDATED_REMOVED_AT
        defaultAuthorityShouldBeFound("removedAt.lessThan=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllAuthoritiesByRemovedAtIsGreaterThanSomething() throws Exception {
        // Initialize the database
        authorityRepository.insert(authority);

        // Get all the authorityList where removedAt is greater than DEFAULT_REMOVED_AT
        defaultAuthorityShouldNotBeFound("removedAt.greaterThan=" + DEFAULT_REMOVED_AT);

        // Get all the authorityList where removedAt is greater than SMALLER_REMOVED_AT
        defaultAuthorityShouldBeFound("removedAt.greaterThan=" + SMALLER_REMOVED_AT);
    }
    ...
    void putNewAuthority() throws Exception {
        ...
        assertThat(testAuthority.getRemovedAt()).isEqualTo(UPDATED_REMOVED_AT);
    }
