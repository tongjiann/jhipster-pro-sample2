...
class OssConfigResourceIT {
    ...
    void createOssConfig() throws Exception {
        ...
        assertThat(testOssConfig.getRemovedAt()).isEqualTo(DEFAULT_REMOVED_AT);
    }
    ...
    @Test
    @Transactional
    void getAllOssConfigsByRemovedAtIsEqualToSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.insert(ossConfig);

        // Get all the ossConfigList where removedAt equals to DEFAULT_REMOVED_AT
        defaultOssConfigShouldBeFound("removedAt.equals=" + DEFAULT_REMOVED_AT);

        // Get all the ossConfigList where removedAt equals to UPDATED_REMOVED_AT
        defaultOssConfigShouldNotBeFound("removedAt.equals=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllOssConfigsByRemovedAtIsNotEqualToSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.insert(ossConfig);

        // Get all the ossConfigList where removedAt not equals to DEFAULT_REMOVED_AT
        defaultOssConfigShouldNotBeFound("removedAt.notEquals=" + DEFAULT_REMOVED_AT);

        // Get all the ossConfigList where removedAt not equals to UPDATED_REMOVED_AT
        defaultOssConfigShouldBeFound("removedAt.notEquals=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllOssConfigsByRemovedAtIsInShouldWork() throws Exception {
        // Initialize the database
        ossConfigRepository.insert(ossConfig);

        // Get all the ossConfigList where removedAt in DEFAULT_REMOVED_AT or UPDATED_REMOVED_AT
        defaultOssConfigShouldBeFound("removedAt.in=" + DEFAULT_REMOVED_AT + "," + UPDATED_REMOVED_AT);

        // Get all the ossConfigList where removedAt equals to UPDATED_REMOVED_AT
        defaultOssConfigShouldNotBeFound("removedAt.in=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllOssConfigsByRemovedAtIsNullOrNotNull() throws Exception {
        // Initialize the database
        ossConfigRepository.insert(ossConfig);

        // Get all the ossConfigList where removedAt is not null
        defaultOssConfigShouldBeFound("removedAt.specified=true");

        // Get all the ossConfigList where removedAt is null
        defaultOssConfigShouldNotBeFound("removedAt.specified=false");
    }

    @Test
    @Transactional
    void getAllOssConfigsByRemovedAtIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.insert(ossConfig);

        // Get all the ossConfigList where removedAt is greater than or equal to DEFAULT_REMOVED_AT
        defaultOssConfigShouldBeFound("removedAt.greaterThanOrEqual=" + DEFAULT_REMOVED_AT);

        // Get all the ossConfigList where removedAt is greater than or equal to UPDATED_REMOVED_AT
        defaultOssConfigShouldNotBeFound("removedAt.greaterThanOrEqual=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllOssConfigsByRemovedAtIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.insert(ossConfig);

        // Get all the ossConfigList where removedAt is less than or equal to DEFAULT_REMOVED_AT
        defaultOssConfigShouldBeFound("removedAt.lessThanOrEqual=" + DEFAULT_REMOVED_AT);

        // Get all the ossConfigList where removedAt is less than or equal to SMALLER_REMOVED_AT
        defaultOssConfigShouldNotBeFound("removedAt.lessThanOrEqual=" + SMALLER_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllOssConfigsByRemovedAtIsLessThanSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.insert(ossConfig);

        // Get all the ossConfigList where removedAt is less than DEFAULT_REMOVED_AT
        defaultOssConfigShouldNotBeFound("removedAt.lessThan=" + DEFAULT_REMOVED_AT);

        // Get all the ossConfigList where removedAt is less than UPDATED_REMOVED_AT
        defaultOssConfigShouldBeFound("removedAt.lessThan=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllOssConfigsByRemovedAtIsGreaterThanSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.insert(ossConfig);

        // Get all the ossConfigList where removedAt is greater than DEFAULT_REMOVED_AT
        defaultOssConfigShouldNotBeFound("removedAt.greaterThan=" + DEFAULT_REMOVED_AT);

        // Get all the ossConfigList where removedAt is greater than SMALLER_REMOVED_AT
        defaultOssConfigShouldBeFound("removedAt.greaterThan=" + SMALLER_REMOVED_AT);
    }
    ...
    void putNewOssConfig() throws Exception {
        ...
        assertThat(testOssConfig.getRemovedAt()).isEqualTo(UPDATED_REMOVED_AT);
    }
