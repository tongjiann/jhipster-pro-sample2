...
class SmsMessageResourceIT {
    ...
    void createSmsMessage() throws Exception {
        ...
        assertThat(testSmsMessage.getRemovedAt()).isEqualTo(DEFAULT_REMOVED_AT);
    }
    ...
    @Test
    @Transactional
    void getAllSmsMessagesByRemovedAtIsEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where removedAt equals to DEFAULT_REMOVED_AT
        defaultSmsMessageShouldBeFound("removedAt.equals=" + DEFAULT_REMOVED_AT);

        // Get all the smsMessageList where removedAt equals to UPDATED_REMOVED_AT
        defaultSmsMessageShouldNotBeFound("removedAt.equals=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllSmsMessagesByRemovedAtIsNotEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where removedAt not equals to DEFAULT_REMOVED_AT
        defaultSmsMessageShouldNotBeFound("removedAt.notEquals=" + DEFAULT_REMOVED_AT);

        // Get all the smsMessageList where removedAt not equals to UPDATED_REMOVED_AT
        defaultSmsMessageShouldBeFound("removedAt.notEquals=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllSmsMessagesByRemovedAtIsInShouldWork() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where removedAt in DEFAULT_REMOVED_AT or UPDATED_REMOVED_AT
        defaultSmsMessageShouldBeFound("removedAt.in=" + DEFAULT_REMOVED_AT + "," + UPDATED_REMOVED_AT);

        // Get all the smsMessageList where removedAt equals to UPDATED_REMOVED_AT
        defaultSmsMessageShouldNotBeFound("removedAt.in=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllSmsMessagesByRemovedAtIsNullOrNotNull() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where removedAt is not null
        defaultSmsMessageShouldBeFound("removedAt.specified=true");

        // Get all the smsMessageList where removedAt is null
        defaultSmsMessageShouldNotBeFound("removedAt.specified=false");
    }

    @Test
    @Transactional
    void getAllSmsMessagesByRemovedAtIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where removedAt is greater than or equal to DEFAULT_REMOVED_AT
        defaultSmsMessageShouldBeFound("removedAt.greaterThanOrEqual=" + DEFAULT_REMOVED_AT);

        // Get all the smsMessageList where removedAt is greater than or equal to UPDATED_REMOVED_AT
        defaultSmsMessageShouldNotBeFound("removedAt.greaterThanOrEqual=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllSmsMessagesByRemovedAtIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where removedAt is less than or equal to DEFAULT_REMOVED_AT
        defaultSmsMessageShouldBeFound("removedAt.lessThanOrEqual=" + DEFAULT_REMOVED_AT);

        // Get all the smsMessageList where removedAt is less than or equal to SMALLER_REMOVED_AT
        defaultSmsMessageShouldNotBeFound("removedAt.lessThanOrEqual=" + SMALLER_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllSmsMessagesByRemovedAtIsLessThanSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where removedAt is less than DEFAULT_REMOVED_AT
        defaultSmsMessageShouldNotBeFound("removedAt.lessThan=" + DEFAULT_REMOVED_AT);

        // Get all the smsMessageList where removedAt is less than UPDATED_REMOVED_AT
        defaultSmsMessageShouldBeFound("removedAt.lessThan=" + UPDATED_REMOVED_AT);
    }

    @Test
    @Transactional
    void getAllSmsMessagesByRemovedAtIsGreaterThanSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where removedAt is greater than DEFAULT_REMOVED_AT
        defaultSmsMessageShouldNotBeFound("removedAt.greaterThan=" + DEFAULT_REMOVED_AT);

        // Get all the smsMessageList where removedAt is greater than SMALLER_REMOVED_AT
        defaultSmsMessageShouldBeFound("removedAt.greaterThan=" + SMALLER_REMOVED_AT);
    }
    ...
    void putNewSmsMessage() throws Exception {
        ...
        assertThat(testSmsMessage.getRemovedAt()).isEqualTo(UPDATED_REMOVED_AT);
    }
